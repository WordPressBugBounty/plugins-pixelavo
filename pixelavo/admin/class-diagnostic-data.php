    ...
    class Pixelavo_Diagnostic_Data {
        ...
        private function __construct() {
            ...
            add_action( 'wp_ajax_pixelavo_diagnostic_data', function () {
                ...
                $agreed = isset( $_POST['agreed'] ) ? sanitize_key( wp_unslash( $_POST['agreed'] ) ) : '' ;
                if( $agreed === 'yes' ){
            ...
            add_action('plugins_loaded', function(){
                $nonce = isset( $_GET['pixelavo_diagnostic_data_nonce'] ) ? sanitize_text_field( wp_unslash($_GET['pixelavo_diagnostic_data_nonce']) ) : '' ;
                if(!wp_verify_nonce( $nonce, 'pixelavo-diagnostic-data-nonce' )){
                    return;
                }
                $agreed  = isset( $_GET['pixelavo_diagnostic_data_agreed'] ) ? sanitize_key( wp_unslash($_GET['pixelavo_diagnostic_data_agreed']) ) : '' ;
        ...
        public function should_show_notice() {
            ...
            foreach ( $sibling_plugins as $plugin_slug => $options ) {
                if ( get_option( $options['agreed'] ) === 'yes' ) {
                    update_option( 'pixelavo_diagnostic_data_agreed', 'yes' );
                    update_option( 'pixelavo_diagnostic_data_notice', 'no' );
                    return false;
                }
            }

            // Ensure only one HT plugin shows the diagnostic notice per request.
            global $pixelavo_diagnostic_notice_owner;
            if ( isset( $pixelavo_diagnostic_notice_owner ) && $pixelavo_diagnostic_notice_owner !== 'pixelavo' ) {
                return false;
            }
            $pixelavo_diagnostic_notice_owner = 'pixelavo';
        ...
        private function show_core_notice() {
            ...
            */
            $message_l2 = sprintf( esc_html__( 'Server information (Web server, PHP version, MySQL version), WordPress information, site name, site URL, number of plugins, number of users, your name, and email address. You can rest assured that no sensitive data will be collected or tracked. %1$sPrivacy Policy%2$s', 'pixelavo' ), '<a target="_blank" href="' . esc_url( $this->privacy_policy ) . '">', '</a>' );

            $button_text_1 = esc_html__( 'Count Me In', 'pixelavo' );
            $button_link_1 = add_query_arg( array( 'pixelavo_diagnostic_data_agreed' => '1', 'pixelavo_diagnostic_data_nonce' => wp_create_nonce( 'pixelavo-diagnostic-data-nonce' ) ) );

            $button_text_2 = esc_html__( 'No thanks', 'pixelavo' );
            $button_link_2 = add_query_arg( array( 'pixelavo_diagnostic_data_agreed' => '0', 'pixelavo_diagnostic_data_nonce' => wp_create_nonce( 'pixelavo-diagnostic-data-nonce' ) ) );
            ?>
            <div class="pixelavo-diagnostic-data-notice">
                <p class="pixelavo-diagnostic-data-message"><?php
                // translators: %1$s: plugin name, %2$s: opening strong tag, %3$s: closing strong tag, %4$s: opening link tag, %5$s: closing link tag
                echo wp_kses_post( sprintf( esc_html__( 'Want to help make %2$s%1$s%3$s even more awesome? Allow %1$s to collect diagnostic data and usage information. (%4$swhat we collect%5$s)', 'pixelavo' ), esc_html( $this->project_name ), '<strong>', '</strong>', '<a href="#" class="pixelavo-diagnostic-data-list-toogle">', '</a>' ) ); ?></p>

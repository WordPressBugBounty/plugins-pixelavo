...
class Pixelavo_Notice_Manager{

    // Remote URL
    const REST_ROUTE_URL = 'https://feed.hasthemes.com/notices';
    // const REST_ROUTE_URL = PIXELAVO_PL_URL;

    // Transient Key
    const TRANSIENT_KEYS = [
        'notice'  => 'pixelavo_notice_info',
    ];

    // API Endpoint
    const API_ENDPOINT = [
        'notice'      => 'pixelavo.json',
    ];
    ...
    public static function get_api_endpoint(){
        if( is_plugin_active('pixelavo-pro/pixelavo-pro.php') && function_exists('pixelavo_pro_notice_endpoint') ){
            return pixelavo_pro_notice_endpoint();
        }
    ...
    public static function delete_transient_cache_data(){
        if ( get_option( 'pixelavo_delete_data_fetch_cache', FALSE ) ) {
